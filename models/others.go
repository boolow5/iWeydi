...
type Follower struct {
	...
	Followee *User  `json:"followee" orm:"rel(fk);on_delete(cascade)"`
	User     *User  `json:"user" orm:"rel(fk);on_delete(cascade)"`
	Topic    *Topic `json:"topic" orm:"rel(fk);on_delete(cascade)"`
}
...
type Knows struct {
	...
	User  *User  `json:"user" orm:"rel(fk);on_delete(cascade)"`
	Topic *Topic `json:"knows" orm:"rel(fk);on_delete(cascade)"`
}
